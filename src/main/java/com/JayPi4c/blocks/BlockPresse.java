...
public class BlockPresse extends BlockContainer
{
    public static final PropertyDirection FACING = PropertyDirection.create("facing", EnumFacing.Plane.HORIZONTAL);
    private final boolean isBurning;
    private static boolean keepInventory;

    public BlockPresse(boolean isBurning)
    {
        super(Material.rock);
        this.setCreativeTab(AppleDimension.tabCustomCrativeTab);
        this.setUnlocalizedName("presse");
    ...
    {
        return Item.getItemFromBlock(AppleDimension.presse);
    }
  ...
  /* private void setDefaultFacing(World worldIn, BlockPos pos, IBlockState state)
    ...
    public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        if (this.isBurning)
        {
            ...
            {
                case WEST:
                    worldIn.spawnParticle(EnumParticleTypes.WATER_WAKE, d0 - d3, d1, d2 + d4, 0.0D, 0.0D, 0.0D, new int[0]);
                    break;
                case EAST:
                    worldIn.spawnParticle(EnumParticleTypes.WATER_WAKE, d0 + d3, d1, d2 + d4, 0.0D, 0.0D, 0.0D, new int[0]);
                    break;
                case NORTH:
                    worldIn.spawnParticle(EnumParticleTypes.WATER_WAKE, d0 + d4, d1, d2 - d3, 0.0D, 0.0D, 0.0D, new int[0]);
                    break;
                case SOUTH:
                    worldIn.spawnParticle(EnumParticleTypes.WATER_WAKE, d0 + d4, d1, d2 + d3, 0.0D, 0.0D, 0.0D, new int[0]);
            }
